...
개발 중 막혔던 부분을 해결하기 위해 아래와 같은 질문들을 확인했습니다.

| 번호 | 질문 내용 |
|-----|---|
| 1 | `app.py`랑 `main.py` 둘 중에 하나만 있어야 하는 거 아니야? 두 개 있어도 돼? |
| 2 | `main.py`가 왜 두 개야? 루트에 있는 거랑 `src/main.py`랑 뭐가 달라? |
| 3 | `src.main` 불러오는 역할만 하면 그냥 `app.py`로 합치는 게 낫지 않아? |
| 4 | 나는 분석 모델을 총 3개 두고 싶은데, 하나는 `Ollama`, 하나는 `Chandra`, 하나는 `gpt-4o`로  |
| 5 | 가이드는 초반에 썼던 거라 코딩 스타일이나 보안/에러 처리는 유지하고, 프로젝트 구조만 3개 모델 쓸 수 있게 수정하고싶은데 |
| 6 | `app.py`에서 `modelSelect` 값으로 `OLLAMA`, `CHANDRA`, `GPT` 나눠도 되는 거야? |
| 7 | 프론트에서 라디오 버튼 선택하면 `formData.append("modelSelect", "CHANDRA")` 이런 식으로 보내면 돼? |
| 8 | 프론트에서 `Chandra` 선택했는데 왜 자꾸 `Ollama`로 연결돼? |
| 9 | `Chandra`를 HuggingFace 방식으로 하려면 뭐 설치해야 돼? |
| 10 | 지금 `Chandra` 할 때 GPU 쓰고 있나? |
| 11 | PyTorch가 지금 CUDA 버전으로 깔린 건지 CPU 버전으로 깔린 건지 어떻게 확인해? |
| 12 | `gpt-4o`가 PNG 이미지는 현재 못 받는 건가? |
| 13 | GPT 코드에서 `data:image/jpeg;base64`로 고정되어있는 것 같은데 PNG 넣으면 잘 작동하도록 어떻게 해? |
| 14 | `tests`는 제대로 작동하는 거야? |
| 15 | `src/core.py` 지웠는데 `tests/test_core.py` 남아 있으면 에러 나는 거 아니야? |
| 16 | `.gitignore` 지금 현재 프로젝트에 맞게 다시 정리해줘 |
| 17 | `.venv`, `__pycache__`, `node_modules`, `.env` 이런 거 GitHub에 올리면 안 되는 거 아니야? |
| 18 | Node.js 실행했는데 3000번 포트 충돌 나면 어떻게 해야 돼? |
| 19 | 이미 GitHub에 올라간 `.venv` 폴더를 뺄 수 있어? |
| 20 | README에는 프로젝트 구조, 실행 방법, API 명세, 실행 화면, 트러블슈팅, 커밋 기록을 어떤 순서로 넣으면 돼? |
| 21 | 중간중간 Git 커밋을 못 했는데 지금이라도 기능별로 나눠서 커밋해도 돼? |